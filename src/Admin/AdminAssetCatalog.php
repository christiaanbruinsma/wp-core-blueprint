...
final class AdminAssetCatalog {
	...
	private static function definition( string $asset_id ): ?array {
		...
		if ( str_starts_with( $asset_id, 'module.' ) ) {
			return [ 'type' => 'module', 'module' => '@cb-core/' . substr( $asset_id, strlen( 'module.' ) ) ];
		}

		if ( 'provider.preferences-media' === $asset_id ) {
			return [ 'type' => 'provider', 'provider' => static function (): void { wp_enqueue_media(); } ];
		}
	...
	private static function enqueue_foundation( string $foundation ): void {
		switch ( $foundation ) {
			case 'icons': UiAssets::enqueue_icons(); break;
			case 'modal': UiAssets::enqueue_modals( UiAssets::MODAL_PRESENTATION_CORE ); break;
			case 'toast': UiAssets::enqueue_toasts( UiAssets::TOAST_PRESENTATION_CORE ); break;
			case 'clipboard': UiAssets::enqueue_clipboard( UiAssets::CLIPBOARD_PRESENTATION_CORE ); break;
			case 'time-picker': UiAssets::enqueue_time_picker( UiAssets::TIME_PICKER_PRESENTATION_CORE ); break;
			case 'object-picker': UiAssets::enqueue_object_picker( UiAssets::OBJECT_PICKER_PRESENTATION_CORE ); break;
			case 'icon-picker': UiAssets::enqueue_icon_picker( UiAssets::ICON_PICKER_PRESENTATION_CORE ); break;
			case 'capability-picker': UiAssets::enqueue_capability_picker( UiAssets::CAPABILITY_PICKER_PRESENTATION_CORE ); break;
			case 'select-picker': UiAssets::enqueue_select_picker( UiAssets::SELECT_PICKER_PRESENTATION_CORE ); break;
			case 'choice-group': UiAssets::enqueue_choice_group( UiAssets::CHOICE_GROUP_PRESENTATION_CORE ); break;
			case 'token-input': UiAssets::enqueue_token_inputs( UiAssets::TOKEN_INPUT_PRESENTATION_CORE ); break;
		}
	}
